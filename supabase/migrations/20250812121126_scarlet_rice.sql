/*
  # Update CogniFlow Workflow Schema

  1. New Tables
    - `cases` - Core case management with milestones
    - `case_milestones` - Track workflow progress
    - `diagnostic_codes` - DSM-5-TR and ICD-11 codes
    - `case_formulations` - Diagnostic formulations
    - `in_between_sessions` - Between-session tracking
    - `resource_library` - Complete resource management
    - `assessment_library` - Psychometric assessments
    - `custom_forms` - Custom form builder
    - `form_assignments` - Assignment tracking
    - `therapeutic_exercises` - Interactive exercises

  2. Functions
    - `create_case_with_milestone()` - Case creation with logging
    - `log_milestone()` - Milestone tracking
    - `get_case_timeline()` - Case progress timeline
    - `get_therapist_profile()` - Complete profile data
    - `calculate_profile_completion()` - Profile completion percentage
    - `get_therapist_insights()` - Dashboard insights
