/*
  # Case Management and Resource Library Enhancement

  1. New Tables
    - `cases` - Case management with unique case numbers
    - `case_milestones` - Track case progress milestones
    - `diagnostic_codes` - DSM-5-TR and ICD-11 diagnostic codes
    - `case_formulations` - Clinical case formulations and diagnostic impressions
    - `in_between_sessions` - Track client activities between therapy sessions

  2. Enhanced Tables
    - `resource_library` - Therapeutic resources and materials
    - `assessment_library` - Psychometric assessment tools

  3. Security
    - Enable RLS on all new tables
    - Add appropriate policies for therapist and client access
    - Ensure data isolation between different therapist practices

  4. Functions
    - Case creation with automatic milestone logging
    - Therapist profile management
    - Progress tracking and insights
    - Timeline generation for case management

  5. Sample Data
    - Diagnostic codes for common mental health conditions
    - Assessment library with validated instruments
    - Resource library with therapeutic materials
*/
...
BEGIN
  ...
  CREATE POLICY "cases_therapist_access" ON cases
  ...
  CREATE POLICY "case_milestones_therapist_access" ON case_milestones
    FOR ALL TO authenticated
    USING (EXISTS (
      SELECT 1 FROM cases 
  ...
  CREATE POLICY "case_milestones_client_read" ON case_milestones
    FOR SELECT TO authenticated
    USING (EXISTS (
      SELECT 1 FROM cases 
      WHERE cases.id = case_milestones.case_id 
  ...
  CREATE POLICY "diagnostic_codes_read_all" ON diagnostic_codes
    FOR SELECT TO authenticated
    USING (is_active = true);

  -- Case formulations policies
  DROP POLICY IF EXISTS "case_formulations_therapist_access" ON case_formulations;
  
  CREATE POLICY "case_formulations_therapist_access" ON case_formulations
    FOR ALL TO authenticated
    USING (therapist_id = auth.uid())
    WITH CHECK (therapist_id = auth.uid());

  -- In between sessions policies
  ...
  CREATE POLICY "in_between_sessions_therapist_access" ON in_between_sessions
    FOR ALL TO authenticated
    USING (EXISTS (
      SELECT 1 FROM cases 
  ...
  CREATE POLICY "in_between_sessions_client_access" ON in_between_sessions
    FOR ALL TO authenticated
    USING (client_id = auth.uid())
